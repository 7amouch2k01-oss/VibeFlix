...
async function loadTrending() {
    ...
    } catch (err) { showError("Failed to load movies."); } finally { showLoading(false); }
}
...
async function searchMovies(query) {
    ...
    try {
        ...
        allMovies = data.results || [];
        renderMovies(allMovies);
    } catch (err) { showError("Search failed."); } finally { showLoading(false); }
}
    ...
    try {
        ...
        const cast = movie.credits?.cast?.slice(0, 10).map(c => `
            <div class="cast-item" onclick="event.stopPropagation(); loadActorMovies(${c.id}, '${c.name.replace(/'/g, "\\'")}')" style="cursor:pointer; min-width:90px; text-align:center;">
                <img src="${c.profile_path ? CONFIG.IMAGE_BASE_URL + c.profile_path : 'https://via.placeholder.com/100x150'}" style="width:70px; height:70px; object-fit:cover; border-radius:50%; margin-bottom:8px; border:2px solid rgba(255,255,255,0.1);">
                <p style="font-size:0.7rem; font-weight:700; color:white; line-height:1.2;">${c.name}</p>
            </div>
        `).join('');

        const similar = movie.similar?.results?.slice(0, 6).map(m => `
            <div class="similar-item" onclick="openDetails(${m.id})" style="cursor:pointer; min-width:130px;">
                <img src="${m.poster_path ? CONFIG.IMAGE_BASE_URL + m.poster_path : 'https://via.placeholder.com/500x750'}" style="width:100%; border-radius:8px; border:1px solid rgba(255,255,255,0.1);">
                <p style="font-size:0.7rem; margin-top:8px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${m.title}</p>
            </div>
        `).join('');
        ...
        document.getElementById('modal-details-content').innerHTML = `
            <div style="position:relative; height:400px; overflow:hidden;">
                ...
                <div style="position:absolute; inset:0; background:linear-gradient(to top, #0a0a0a 10%, transparent 80%); padding:40px; display:flex; flex-direction:column; justify-content:flex-end;">
                    <h2 style="font-size: clamp(1.8rem, 4vw, 3rem); font-weight:900; letter-spacing:-1px; margin-bottom:15px;">${movie.title}</h2>
                    <div style="display:flex; gap:12px; flex-wrap:wrap;">
                        <span style="background:var(--accent-color); padding:4px 12px; border-radius:6px; font-weight:800; font-size:0.75rem;">${movie.release_date.split('-')[0]}</span>
                        <span style="background:rgba(255,255,255,0.1); padding:4px 12px; border-radius:6px; font-weight:800; font-size:0.75rem;">★ ${movie.vote_average.toFixed(1)}</span>
                        <span style="background:rgba(255,255,255,0.1); padding:4px 12px; border-radius:6px; font-weight:800; font-size:0.75rem;">${movie.runtime} MIN</span>
                    </div>
                </div>
            </div>
            <div style="padding:40px; display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:40px;">
                <div>
                    <h3 style="color:var(--accent-color); text-transform:uppercase; font-size:0.75rem; letter-spacing:2px; margin-bottom:15px;">Storyline</h3>
                    <p style="font-size:1rem; color:var(--text-secondary); line-height:1.6; margin-bottom:30px;">${movie.overview || 'No description available.'}</p>
                    
                    <h3 style="color:var(--accent-color); text-transform:uppercase; font-size:0.75rem; letter-spacing:2px; margin-bottom:20px;">Top Cast</h3>
                    <div style="display:flex; gap:15px; overflow-x:auto; padding-bottom:15px; scrollbar-width:none;">${cast || 'Cast info unavailable.'}</div>
                </div>
                <div>
                    <h3 style="color:var(--accent-color); text-transform:uppercase; font-size:0.75rem; letter-spacing:2px; margin-bottom:15px;">Official Trailer</h3>
                    <div style="position:relative; width:100%; padding-top:56.25%; border-radius:12px; overflow:hidden; background:#000; border:1px solid rgba(255,255,255,0.1); margin-bottom:25px;">
                        ${trailer ? `
                            <iframe style="position:absolute; inset:0; width:100%; height:100%; border:none;" src="https://www.youtube.com/embed/${trailer.key}?rel=0" allowfullscreen></iframe>
                        ` : `<div style="position:absolute; inset:0; display:flex; align-items:center; justify-content:center; color:var(--text-secondary); font-size:0.8rem;">Trailer unavailable</div>`}
                    </div>
                    ...
                    <h3 style="color:var(--accent-color); text-transform:uppercase; font-size:0.75rem; letter-spacing:2px; margin-top:35px; margin-bottom:20px;">Similar Movies</h3>
                    <div style="display:flex; gap:12px; overflow-x:auto; padding-bottom:10px; scrollbar-width:none;">${similar || 'No recommendations.'}</div>
                </div>
            </div>
        `;
        ...
        card.innerHTML = `
            ...
            <div class="movie-info">
                ...
                <div class="movie-card-meta">
                    ...
                    <span style="color:var(--accent-color); font-weight:700; font-size:0.7rem;">${inWatchlist ? 'IN WATCHLIST' : ''}</span>
                </div>
            </div>
        `;
...
function updateWatchlistUI() { document.querySelectorAll('.watchlist-count').forEach(c => c.textContent = watchlist.length); }
...
function renderWatchlist() {
    ...
    watchlist.forEach(movie => {
        ...
        card.className = 'movie-card';
        card.innerHTML = `
            <div class="poster-container"><img src="${movie.poster_path ? CONFIG.IMAGE_BASE_URL + movie.poster_path : 'https://via.placeholder.com/500x750'}" alt="${movie.title}"></div>
